...
class TestEdgeDataBridge(TenderBaseWebTest):
    ...
    #     t_date_modified = datetime.datetime.utcnow().isoformat()
    #     mock_tender['data']['dateModified'] = t_date_modified
    #     bridge.get_items_list = MagicMock(return_value=[[tid, datetime.datetime.utcnow().isoformat()]])
    #     bridge.run()
    #     x = log_string.getvalue().split('\n')
    ...
    @patch('openprocurement.edge.databridge.get_items')
    def test_get_items_list(self, mock_get_items):
        tid = uuid.uuid4().hex
        t_date_modified =  datetime.datetime.utcnow().isoformat()
        mock_get_items.return_value = [{'id': tid, 'dateModified': t_date_modified}]
        bridge = EdgeDataBridge(self.config)
        for tender_id, date_modified in bridge.get_items_list():
