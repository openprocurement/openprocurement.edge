...
class TestEdgeDataBridge(TenderBaseWebTest):
    ...
    def setUp(self):
        self.config = deepcopy(DEFAULT_CONFIGS)
        server = Server(self.config['main']['storage']['couch_url'])
        ...
        couchdb_url = self.config['main']['storage']['couch_url'] \
            + '/' + self.config['main']['storage']['db_name']
        ...
        try:
            self.storage.db.save(validate_doc)
        except Exception:
            pass
    ...
    def tearDown(self):
        ...
        try:
            server = Server(self.config['main']['storage'].get('couch_url') or
                            'http://127.0.0.1:5984')
    ...
    def test_init(self):
        ...
        self.assertIn('couch_url', bridge.config['main']['storage'])
        self.assertIn('db_name', bridge.config['main']['storage'])
        self.assertEqual(len(bridge.storage.server.uuids()[0]), 32)
        ...
        self.config['main']['storage']['couch_url'] = 'http://127.0.0.1:5987'
        with self.assertRaises(DataBridgeConfigError) as e:
            bridge = EdgeDataBridge(self.config)
        self.assertEqual(e.exception.message, 'Connection refused')

        del bridge
        self.config['main']['storage']['couch_url'] = 'http://127.0.0.1:5984'

        try:
            server = Server(self.config['main']['storage'].get('couch_url'))
            del server[self.config['main']['storage']['db_name']]
        except:
            pass
        test_config = {}
        ...
        test_config['main']['storage']['db_name'] = 'public'
        test_config['main']['resources_api_version'] = "0"
        test_config['main']['public_resources_api_server'] \
            = 'https://lb.api-sandbox.openprocurement.org'

        # Create EdgeDataBridge object with deleting config variables
        # step by step
        ...
        server = Server(test_config['main'].get('couch_url') or
                        'http://127.0.0.1:5984')
        del server[test_config['main']['storage']['db_name']]
    ...
    def test_send_bulk(self):
        ...
        bridge = EdgeDataBridge(self.config)
        bridge.storage.db.view = MagicMock(return_value=return_value)
        ...
        bridge.storage.db.view.side_effect = [Exception(), Exception(),
                                      Exception('test')]
        input_dict = {}
    ...
    def test_fill_resource_items_queue(self):
        ...
        bridge.storage.db.view = MagicMock(return_value=view_return_list)
        with patch('__builtin__.True', AlmostAlwaysTrue(1)):
            bridge.fill_resource_items_queue()
        self.assertEqual(bridge.resource_items_queue.qsize(), 1)
    ...
    def test_config_get(self):
        ...
        bridge = EdgeDataBridge(test_config)
        api_version_config = bridge.config_get('resources_api_version')
        self.assertEqual(api_version_config,
                         test_config['main']['resources_api_version'])

        del bridge.config['main']['resources_api_version']
        api_version_config = bridge.config_get('resources_api_version')
        self.assertEqual(api_version_config, None)
        server = Server(test_config['main'].get('couch_url') or
                        'http://127.0.0.1:5984')
